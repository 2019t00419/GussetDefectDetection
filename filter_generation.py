...
def filter_generation(img, df,imgc):
    '''
    theta_min = 20
    theta_max = 30
    ...
    h_channel = hsv[:, :, 0]
    s_channel = hsv[:, :, 1]
    v_channel = hsv[:, :, 2]# Invert the colors of the saturation channel
    s_channel_inverted = 255 - s_channel
    v_channel_inverted = 255 - v_channel

    results['h_channel'] = h_channel.reshape(-1)
    results['s_channel'] = s_channel.reshape(-1)
    results['v_channel'] = v_channel.reshape(-1)
    results['v_channel_inverted'] = v_channel_inverted.reshape(-1)
    results['s_channel_inverted'] = s_channel_inverted.reshape(-1)
    #cv.imshow("h_channel", h_channel)
    #cv.imshow("s_channel", s_channel)
    #cv.imshow("v_channel", v_channel)
    #cv.imshow("v_channel_inverted", v_channel_inverted)
    #cv.imshow("s_channel_inverted", s_channel_inverted)
